...
import { FormComponent, StepOneProps } from '../lib/types';

export const StepOne = ({ data, onDataChange }: StepOneProps) => {
    const [formComponents, setFormComponents] = useState<FormComponent[]>(data?.formComponents || []);
    ...
    const addComponent = (type: string) => {
        ...
        const newComponents = [...formComponents, newComponent];
        setFormComponents(newComponents);
        onDataChange({ formComponents: newComponents });
    };

    const removeComponent = (id: string) => {
        const newComponents = formComponents.filter((comp) => comp.id !== id);
        setFormComponents(newComponents);
        onDataChange({ formComponents: newComponents });
    };

    const updateComponent = (updatedComponent: FormComponent) => {
        const newComponents = formComponents.map((comp) => 
